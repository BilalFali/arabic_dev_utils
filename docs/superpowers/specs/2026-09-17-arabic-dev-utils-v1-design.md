...
- `ArabicDirectionality({required Widget child, required String basedOn})` — wraps `Directionality`, setting `textDirection` from `ArabicDirection.isRtl(basedOn)` (first-strong-character resolution — the standard approach used by browsers' `dir="auto"` and ICU's bidi guessing), not from `dominantDirection`. This matters for realistic Arabic-dominant sentences with an embedded Latin word or number (e.g. `'أحمد يعمل في London'`): such text should still read as RTL overall, because it opens with an Arabic character, and Flutter's own bidi text layout places the embedded Latin run correctly within that RTL paragraph regardless of the paragraph's base direction. An earlier version of this widget used `dominantDirection` and forced `TextDirection.ltr` for any text containing both scripts — this produced the wrong direction for the common case above and was corrected after being caught in live testing on Android. Text with no strong-directional character (digits/punctuation only, or an empty string) resolves to `TextDirection.ltr`. The enum was renamed from `TextDirection` to `ArabicTextDirection` to avoid colliding with Flutter's own `TextDirection` type for consumers who import both unprefixed.
